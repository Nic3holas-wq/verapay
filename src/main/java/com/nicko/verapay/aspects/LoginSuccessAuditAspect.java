...
import com.nicko.verapay.dto.LoginResponseDto;
...
public class LoginSuccessAuditAspect {
    ...
    @AfterReturning(
            pointcut = "execution(* com.nicko.verapay.auth.AuthController.apiLogin(..))",
            returning = "response"
    )
    public void logSuccessfulLogin(JoinPoint joinPoint, Object response) {
        ...
        if (loginResponse.user() != null) {
            String username = loginResponse.user().email(); // or username
